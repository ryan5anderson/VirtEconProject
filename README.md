...
## Querying Without the Frontend (Optional)

You can interact with the backend API directly using tools like `curl` or Postman without starting the frontend.
...
This is useful for quickly testing the backend or integrating with other tools and services.

---

## OpenSearch (RAG) Access Notes

The chatbot uses a Retrieval-Augmented Generation (RAG) architecture with **AWS OpenSearch** to look up relevant economic context before answering.

To enable RAG functionality:

* You must have access to a configured **AWS OpenSearch cluster** containing pre-loaded embeddings.
* You must set **valid AWS environment variables** as shown earlier in the setup, or switch to Basic Auth if configured.

If no OpenSearch connection is configured or accessible, the chatbot will not be able to retrieve additional context and may return fallback responses.
